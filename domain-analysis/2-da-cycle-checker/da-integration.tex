...
Each clock tick a source can generate a packet, a sink can destroy one and a queue can accept
...
Would this be the case, a packet entering the combinatoric cycle could revolve endlessly in
this loop. The packet would never arrive at its destination and the simulation could not
...
\begin{enumerate}
 ...
 \begin{itemize}
  ...
  tools (XMASComponent and Port). (See also section~\ref{sec:extensions-visitor-design-tool})
 \end{itemize}
...
efficient lookup of the network objects by id. However, since the second method is capable
