...
struct Number {
    ...
    Number(float f) : type(isnan(f) ? ERROR : FLOAT), floatValue(f) {
    }
    ...
    Number pow(const Number &n) const {
        if (isError() || n.isError())
            return Number(ERROR);
        return (float)::powf(getFloat(), n.getFloat());
    }
    Number sqrt() const {
        if (isError())
            return Number(ERROR);
        if (isInt()) {
            int candidate = (float)sqrtf(getFloat());
            if (candidate*candidate == getInt())
                return candidate;
        }
        return (float)::sqrtf(getFloat());
    }
    Number log2() const {
        if (isError())
            return Number(ERROR);
        return (float)::log2f(getFloat());
    }
    Number sin() const {
        if (isError())
            return Number(ERROR);
        return (float)::sinf(getFloat());
    }
    Number cos() const {
        if (isError())
            return Number(ERROR);
        return (float)::cos(getFloat());
    }
    Number tan() const {
        if (isError())
            return Number(ERROR);
        return (float)::tan(getFloat());
    }
    ...
    */
    ...
    TokenBase *recognise() {
        ...
        if (StringParser(c).accept("pi").remainder(c)) {
            numberToken.value = Number((float)M_PI);
            return &numberToken;
        }
        if (StringParser(c).accept("e").remainder(c)) {
            numberToken.value = Number((float)M_E);
            return &numberToken;
        }
        ...
        int intValue;
        if (StringParser(c).parse(intValue).remainder(c)) {
...
struct NumberExpression {
    ...
    static PS negativeConstant(PS cont, UserData userData) {
        ...
        return cont()
                ...
                .perform(primary)
                .modify([](Number &value, UserData){ value = -value; return 0; });
    }
