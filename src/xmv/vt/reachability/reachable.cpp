...
bool isReachable(const std::set<XMASComponent *> &allComponents,
  ...
  bool reverse = flags & reachable_reverse;

  auto generic_transitions = compute_transitions(allComponents, reverse);

  XMASState start_state = reverse ? s : XMASState{allComponents};
  size_t start_hash = start_state.hash();
  size_t s_hash = s.hash();
  
  auto reached_states = std::unordered_set<size_t>{};
  reached_states.insert(start_hash);
  auto next_states = std::deque<XMASState>{start_state};
  ...
  while (!next_states.empty()) {
    auto s = std::move(next_states.front());
    next_states.pop_front();

    for (auto &gt : generic_transitions) {
      for (auto trans : gt.transitions(s)) {
        if (!trans.is_enabled()) {
          continue;
        }
        ...
        if (reverse && s_prime.initial_state()) {
          reachable = true;
          goto finally;
        } else if (!reverse && s_hash == s_prime_hash) {
          reachable = true;
          goto finally;
        }

        if (set_contains(reached_states, s_prime_hash)) continue;
  ...
  std::cerr << "Wall clock time: "
            << std::chrono::duration_cast<std::chrono::duration<float>>(
                   t_end - t_start).count() << " s" << std::endl;
  return reachable;
}
