...
protected:
    ...
    size_t len;
public:
    const static size_t UNKNOWN_LENGTH = std::numeric_limits<size_t>::max();
...
constexpr StableHashedString operator "" _HS(const char * value, size_t size) {
    return StableHashedString(value, size);
}
