...
private:
    ...
    std::size_t size:31; // including this struct
...
class MemoryPoolStatus {
    ...
    int available;
public:
    MemoryPoolStatus(MemoryPoolPage* start = nullptr, MemoryPoolPage* nextPage = nullptr) : current(start), nextPage(nextPage), available(0) {};
};
