...
TEST(Parser, JSONParserState) {
    ...
    */
    ...
    retval = ParseJSON("[]", mp);
    EXPECT_TRUE(retval);
    EXPECT_EQ(JSONData(JSONData::AllocateVector(mp)), retval.result());

    retval = ParseJSON("{}", mp);
    EXPECT_TRUE(retval);
    EXPECT_EQ(JSONData(JSONData::AllocateMap(mp)), retval.result());
    ...
    */

    {
        ...
        JSONData expectedObject(expectedMap);
...

    {
        ...
        expectedMap["foo"] = expectedVector;
        JSONData expectedObject(expectedMap);
        EXPECT_EQ(expectedObject, retval.result());
    }
