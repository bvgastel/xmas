...
class SymbolicRangeField : public SymbolicPacketField {
    ...
    bool complement;
public:
    SymbolicRangeField(const SymbolicRangeField &copy) : SymbolicPacketField(FastType<SymbolicRangeField>::value), min(copy.min), max(copy.max), complement(copy.complement) {
        //std::cout << "copy of SymbolicRangeField" << std::endl;
    }
    SymbolicRangeField(SymbolicRangeField &&copy) : SymbolicPacketField(FastType<SymbolicRangeField>::value), min(copy.min), max(copy.max), complement(copy.complement) {
        //std::cout << "move of SymbolicRangeField" << std::endl;
    }
    SymbolicRangeField() : SymbolicPacketField(FastType<SymbolicRangeField>::value), min(0), max(0), complement(false) {
    }
    SymbolicRangeField(int min, int max, bool complement = false) : SymbolicPacketField(FastType<SymbolicRangeField>::value), min(min), max(max), complement(complement) {
    }
