...
unsigned int String::hash() const {
    //return CrapWow((const unsigned char *)ptr, len, 33);
    /* djb2 algorithm */
    unsigned int hash = 5381;
    const unsigned char *end = (const unsigned char *)ptr + len;
    const unsigned char *begin = (const unsigned char *)ptr;
    while (begin < end) {
...
unsigned int String::hashIgnoreCase() const {
    //return SuperFastHash(ptr, len);
    /* djb2 algorithm */
    unsigned int hash = 5381;
    const unsigned char *end = (const unsigned char *)ptr + len;
    const unsigned char *begin = (const unsigned char *)ptr;
    while (begin < end) {
        unsigned char c = toUpper(*begin++);
        hash = hash*33 ^ c;
    }
    return hash;
}
