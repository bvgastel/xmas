...
class DispatchLambdaWrapper : public DispatchOperation {
    T lambda;
public:
    DispatchLambdaWrapper(T lambda) : lambda(lambda) {
    }
    void execute() {
        lambda();
        delete this;
...
DispatchLambdaWrapper<T> *DispatchLambda(T lambda) {
    return new DispatchLambdaWrapper<T>(lambda);
}
...
protected:
    ...
    SpinLock m; // all variables below are protected by the spinlock
    typename Dispatcher<DispatchObject>::DispatchObjectQueue q[numberOfPriorities];
    int executing; // currently executing
    const char *desc;
protected:
    DispatcherFor(const char *description, int maximumExecuting) : maximumExecuting(maximumExecuting), m(), q(), executing(0), desc(description) {
