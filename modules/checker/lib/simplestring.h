...
protected:
    ...
    size_t len;
public:
    constexpr inline String(const char *pointer, size_t len = std::numeric_limits<size_t>::max()) : ptr(pointer), len(len < std::numeric_limits<size_t>::max() ? len : pointer ? strlen(pointer) : 0) {
    }
    //constexpr inline String(const char *pointer, ssize_t len = std::numeric_limits<ssize_t>::max()) : ptr(pointer), len(len < std::numeric_limits<ssize_t>::max() ? len : pointer ? strlen(pointer) : 0) {
    //}
    inline String(const std::string &str) : ptr(str.c_str()), len(str.length()) {
    }
...
public:
    constexpr HashedString(const String &str) : str(str), hashCache(hash(str)) {
    }
    constexpr HashedString(const char * value, size_t size) : str(value, size), hashCache(hash(str)) {
    }
    template <size_t N>
    constexpr inline HashedString(const char (&str)[N]) : str(String(str, N-1)), hashCache(hash(this->str)) {
    }
...
template <class T,
         int bufferSize = std::numeric_limits<T>::digits10+1+std::numeric_limits<T>::is_signed,
         int base = 10>
...
class EscapeOperation : public StringOperation {
    const FirstString &a;
public:
    constexpr EscapeOperation(const FirstString &a) : a(a) {
    }
    size_t writeTo(char * __restrict__ buffer, size_t bufferLength, off_t offset) const {
...
constexpr String operator "" _S(const char * value, size_t size) {
    return String(value, size);
}

constexpr HashedString operator "" _HS(const char * value, size_t size) {
    return HashedString(value, size);
}
...
template <class T>
struct lessIgnoreCase : std::binary_function<T, T, bool> {
    ...
    struct compare : public std::binary_function<unsigned char,unsigned char,bool>
    {
    ...
    bool operator() (const T & lhs, const T & rhs) const {
        ...
        if (len != rhs.length())
            return len < rhs.length();
      return getPointer(lhs) != getPointer(rhs) && std::lexicographical_compare(lhs.begin(), lhs.end(), rhs.begin(), rhs.end(), compare());
    }
};
