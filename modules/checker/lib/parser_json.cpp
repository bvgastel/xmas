...
struct JSONParserState {
    ...
    static PS objectOp(PS cont, UserData userData) {
        return cont().store([](UserData userData){
            return JSONData(JSONData::AllocateMap(userData), userData);
        })
    ...
    static PS arrayOp(PS cont, UserData userData) {
        return cont().store([](UserData userData) -> JSONData {
            return {JSONData::AllocateVector(userData), userData};
        })
...
void JSONData::clear() {
    if (!mp) {
        if (type == JSONArray && array)
            delete array;
        else if (type == JSONObject && object)
            delete object;
    }
    type = JSONNull;
}
...
JSONData& JSONData::operator=(const JSONData &b) {
    ...
    if (type == JSONString)
        ...
        array = new (mp) Vector(*b.array);
    else if (type == JSONObject)
        object = new (mp) Map(*b.object);
    return *this;
}
...
JSONData::JSONData(const JSONData &b) noexcept : type(b.type), mp(b.mp) {
    //if (type != JSONNull)
...
JSONData::JSONData(JSONData &&b) noexcept : type(b.type), mp(b.mp) {
