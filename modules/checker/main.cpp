...
void connectSpidergonNodesRange(std::map<std::string, XMASComponent*> &allComponents, int i, int total) {
    ...
    connect(((XMASSwitch*)allComponents["switchLeft"_S + i])->a, ((XMASQueue*)allComponents["incommingRight"_S + left])->i);
    connect(((XMASSwitch*)allComponents["switchLeft"_S + i])->b, ((XMASQueue*)allComponents["incommingLeft"_S + right])->i);
    connect(((XMASSwitch*)allComponents["switchThis"_S + i])->b, ((XMASQueue*)allComponents["incommingAcross"_S + across])->i);
}


void specSpidergon(std::map<std::string, XMASComponent*> &allComponents, int i, int total) {
    XMASSource *source = dynamic_cast<XMASSource*>(allComponents["source"_S + i]);
    for (int j = 0; j < total; ++j) {
        if (j != i) {
            XMASSink *sink = dynamic_cast<XMASSink*>(allComponents["sink"_S + j]);
...
void SpidergonTest(int size, bool showSinks, bool showAll) {
    ...
    if (showSinks) {
        for (int i = 0; i < total; ++i) {
            XMASSink *sink = (XMASSink *)components["sink"_S + i];
...
void connectMeshNodes(std::map<std::string, XMASComponent*> &allComponents, int x, int y, int width, int height) {
    if (x > 0)
        connect(((XMASQueue*)allComponents["out_east"_S + (x-1) + "_" + y])->o, ((XMASQueue*)allComponents["in_west"_S + x + "_" + y])->i);
    if (x < width-1)
        connect(((XMASQueue*)allComponents["out_west"_S + (x+1) + "_" + y])->o, ((XMASQueue*)allComponents["in_east"_S + x + "_" + y])->i);
    if (y > 0)
        connect(((XMASQueue*)allComponents["out_south"_S + x + "_" + (y-1)])->o, ((XMASQueue*)allComponents["in_north"_S + x + "_" + y])->i);
    if (y < height-1)
        connect(((XMASQueue*)allComponents["out_north"_S + x + "_" + (y+1)])->o, ((XMASQueue*)allComponents["in_south"_S + x + "_" + y])->i);
}
...
void specMesh(std::map<std::string, XMASComponent*> &allComponents, int x, int y, int width, int height, MemoryPool &mp) {
    if (x >= width/2)
        return;
    XMASSource *source = dynamic_cast<XMASSource*>(allComponents["in_local"_S + x + "_" + y]);
    XMASSink *sink = dynamic_cast<XMASSink*>(allComponents["out_local"_S + x + "_" + y]);
...
void MeshTest(int size, bool showSinks, bool showAll) {
    ...
    if (showSinks)
        for (int x = 0; x < width; ++x)
            for (int y = 0; y < height; ++y) {
                XMASSink *sink = (XMASSink *)components["out_local"_S + x + "_" + y];
                if (sink) {
...
void TestFile(const std::string &filename, bool showAll) {
    ...
    auto components = Parse(filename, mp);
