...
public:
    ...
    TokenBase *recognise() {
        ...
        if (sawIf) {
            ...
            std::cerr << "condition: " << variable.value << std::endl;
            sawIf = false;
            sawThen = true;
            return &variable;
        }
        ...
        if (c[0] == MUL || c[0] == PLUS || c[0] == MIN || c[0] == COMMA_OP || c[0] == SET_OPEN || c[0] == SET_CLOSE || c[0] == SET_FIELD || c[0] == OPEN || c[0] == CLOSE) {
            opToken.value = c[0];
            c = c.substring(1);
            //std::cout << "token op: " << opToken.value << std::endl;
            return &opToken;
...
struct PacketFunction {
    ...
    static PExprExpr plusOp(PExprExpr cont, UserData userData) {
        return cont().accept(PacketFunctionLexer::PLUS).process(primary, [](Expr &a, Expr &&b, UserData userData) {
    ...
    static PExprExpr minOp(PExprExpr cont, UserData userData) {
        return cont().accept(PacketFunctionLexer::MIN).process(primary, [](Expr &a, Expr &&b, UserData userData) {
...
template <class T>
T *insert(std::map<String, XMASComponent*> &allComponents, const std::string &name) {
...
std::map<String, XMASComponent *> Parse(const std::string &filename, MemoryPool &mp)
{
    struct stat st;
    if (stat(filename.c_str(), &st)) {
        std::cerr << "error: " << strerror(errno) << std::endl;
        return std::map<String, XMASComponent *>();
    }
    ...
    if (read(fd, buffer, st.st_size) != st.st_size) {
        std::cerr << "wrong Interval of bytes read" << std::endl;
        abort();
    }
    close(fd);

    String current(buffer, st.st_size);
    ...
    for (auto &jsonComponent : json["NETWORK"]) {
        //std::cout << jsonComponent["id"] << ": " << jsonComponent << std::endl;
        String name = jsonComponent["id"];
        HashedString type = jsonComponent["type"];
        if (type == "source"_HS) {
            insert<XMASSource>(retval, name);
        } else if (type == "sink"_HS) {
            insert<XMASSink>(retval, name);
        } else if (type == "queue"_HS) {
            insert<XMASQueue>(retval, name);
        } else if (type == "function"_HS) {
            insert<XMASFunction>(retval, name);
        } else if (type == "xswitch"_HS) {
            insert<XMASSwitch>(retval, name);
        } else if (type == "merge"_HS) {
            insert<XMASMerge>(retval, name);
        } else if (type == "xfork"_HS) {
            insert<XMASFork>(retval, name);
        } else if (type == "join"_HS) {
            insert<XMASJoin>(retval, name);
        } else {
